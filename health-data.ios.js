        ...
        return new Promise(function (resolve, reject) {
            ...
            types.forEach(function (t) {
                authorized = authorized &&
                    _this.healthStore.authorizationStatusForType(_this.resolveDataType(acceptableDataTypes[t.name])) === 2;
            });
            resolve(authorized);
        });
    };
    HealthData.prototype.requestAuthorization = function (types) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            types.filter(function (t) { return t.accessType === "read" || t.accessType === "readAndWrite"; })
                .forEach(function (t) { return readDataTypes.addObject(_this.resolveDataType(acceptableDataTypes[t.name])); });
            types.filter(function (t) { return t.accessType === "write" || t.accessType === "readAndWrite"; })
                .forEach(function (t) { return writeDataTypes.addObject(_this.resolveDataType(acceptableDataTypes[t.name])); });
        ...
        var query = HKSampleQuery.alloc().initWithSampleTypePredicateLimitSortDescriptorsResultsHandler(objectType, predicate, null, sortBy, function (query, listResults, error) {
            if (listResults) {
                ...
                for (var index_1 = 0; index_1 < listResults.count; index_1++) {
                    ...
                    var startDate = sample.startDate, endDate = sample.endDate, source = sample.source;
                    var resultItem = {
                        source: source.name,
                        unit: opts.unit,
                        start: startDate,
    ...
    HealthData.prototype.monitorData = function (dataType, opts) {
        ...
        var query = HKObserverQuery.alloc().initWithSampleTypePredicateUpdateHandler(objectType, null, function (observerQuery, handler, error) {
            if (error) {
                ...
                opts.onUpdate(function () { return handler(); });
            }
        });
