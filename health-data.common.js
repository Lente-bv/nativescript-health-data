    ...
    Common.prototype.aggregate = function (parsedData, aggregateBy) {
        ...
        if (aggregateBy) {
            ...
            if (aggregateBy === "sourceAndDay") {
                ...
                parsedData.forEach(function (item) { return distinctSources_1.add(item.source); });
                distinctSources_1.forEach(function (source) { return _this.aggregateData(parsedData.filter(function (item) { return item.source === source; }), aggregateBy, result_1); });
            }
            else {
